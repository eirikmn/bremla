...
linrampfitter = function(object,control.linramp,print.progress=FALSE){
  ...
  timepoints = round((df_event$xx - df_event$xx[1])/(df_event$xx[n]-df_event$xx[1])*(n-1)+1,digits=4)
  ypoints = df_event$yy*control.linramp$rescale.y.factor
  # if(control.linramp$rescale.y.factor){
  ...
  model.rgeneric = inla.rgeneric.define(rgeneric.uneven.AR1,n=n,
                                        ...
                                        tslutt=timepoints[n],
                                        ystart=y[1],
                                        # rescale.y=FALSE,#control.linramp$rescale.y,
                                        timepoints = timepoints)
  formula = y ~ -1+ f(idx, model=model.rgeneric)

  r = inla(formula,family="gaussian", data=data.frame(y=ypoints,idx=as.integer(df0$time)),
           control.mode=list(theta=init,
                             restart=TRUE),
           num.threads = control.linramp$ncores,
           verbose=control.linramp$verbose,
  ...
  y0=inla.emarginal(function(x)x,r$marginals.hyperpar$`Theta3 for idx`);
  dy=inla.emarginal(function(x)x,r$marginals.hyperpar$`Theta4 for idx`);
  #rho = inla.emarginal(function(x)2/(1+exp(-x))-1,r$marginals.hyperpar$`Theta5 for idx`);
  rho = inla.emarginal(function(x)exp(-1/exp(x)),r$marginals.hyperpar$`Theta5 for idx`);
  sigma = inla.emarginal(function(x)1/sqrt(exp(x)),r$marginals.hyperpar$`Theta6 for idx`)
  muvek = linramp(timepoints,t0=t0,dt=dt,y0=y0,dy=dy)
  ...
  y0marg0[,1] = y0marg0[,1]/control.linramp$rescale.y.factor
  dymarg0[,1] = dymarg0[,1]/control.linramp$rescale.y.factor
  # if(control.linramp$rescale.y){
  ...
  if(control.linramp$nsims>0){
    ...
    for(i in 1:control.linramp$nsims){
      t01 = hpars[i,3]
      dt1 = hpars[i,4]
      y01 = hpars[i,1]/control.linramp$rescale.y.factor
      dy1 = hpars[i,2]/control.linramp$rescale.y.factor
      # if(control.linramp$rescale.y){
  ...
  object$linramp$.args = list(nsims=control.linramp$nsims,
                              depth.reference=control.linramp$depth.reference,
                              label=control.linramp$depth.label,
                              rescale.y.factor=control.linramp$rescale.y.factor)
  object$time$linramp=time.total
