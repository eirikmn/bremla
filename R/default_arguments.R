...
#' \describe{
#'   \item{\code{noise}}{Character which noise process is assumed for the residuals. \code{"iid"}, \code{"ar1"} and \code{"ar2"} are currently. However, if synchronization is to be performed only \code{"ar1"} is currently supported. Default input is \code{"ar1"}.}
#'   \item{\code{method}}{Character describing which method should be used to fit the process. Only \code{inla} is currently supported, but more will hopefully come in the future.}
#'   \item{\code{verbose}}{Boolean telling whether or not the \code{inla}-program should run in verbose mode. Default \code{FALSE}.}
...
#'   \item{\code{improve.fixed}}{Boolean indicating whether or not the least squares estimate of the fixed values should be used as initial values for the optimization procedure in INLA. This feature has not been extensively tested so it might be unstable. Default \code{FALSE}}
#'   \item{\code{ncores}}{Integer used to set the number of cores to be used in the \code{inla} fitting procedure. Default value is \code{1}.}
#'   \item{\code{transform}}{Character indicating which transformation should be applied to the observations. Can be \code{"log"} or \code{"identity"}. If synchronization is intended then only \code{"identity"} is currently supported, which is also the default value here.}
#'   \item{\code{rgeneric}}{List containing necessary arguments for specifying a custom model for the noise component of the layer increments using the \code{rgeneric} framework of \code{INLA}. Note that this alternative generally performs less well than using already implemented model components in \code{INLA} used when \code{noise} is specified. Must include:
#'   \describe{
#'       \item{\code{from.theta}}{List of function specifying transformation from internal to original parameter scaling. Function of \code{theta} (internal parameters). \code{length(from.theta)} must correspond to \code{length(theta)}. This is only used for transforming the parameters and can not be used in the functions below.}
#'       \item{\code{param.names}}{Vector of names for parameters (optional).}
#'       \item{\code{Q}}{Function that computes the precision matrix of the noise model. Must be a function of \code{theta}, \code{n} and \code{ntheta}, and must return a sparse matrix.}
#'       \item{\code{log.prior}}{Function that computes the logarithm of the prior of the internal parameter \code{theta}. Must be a function of \code{theta}, \code{n} and \code{ntheta} (even if they are not used).}
#'   }
#'   }
#' }
...
#'    \item \code{proxy} Numeric giving the proxy data to perform the linear ramp model fit. Default value is \code{NULL} meaning that it must be provided by the user.
#'    \item \code{interval} Interval describing the window of \code{proxy} to perform the linear ramp model fit. Default value is \code{NULL} so it must be given by the user.
#'    \item \code{interval.unit} Character giving which axis is used for interval. Can be \code{"depth"} (default), \code{"age"} or \code{"index"}.
#'    \item \code{depth.label} Character describing what label to place on x-axis (depth) in a future \code{plot}. Default value is \code{""}.
#'    \item \code{depth.ref} Numeric giving a reference value for the depth which will be included in a future \code{plot}.
#'    \item \code{opt.params} Numeric describing initial values for \code{\link{optim}} function used to find good initial values for the \code{inla} fitting procedure. If this is set to \code{NULL} (default) the code will provide its own initial values.
#'    \item \code{compute.t1} Boolean indicating whether or not transition end point \code{t1} should be computed. Default \code{TRUE}.
#'    \item \code{nsims} Integer giving how many Monte Carlo samples should be used to compute transition end point \code{t1} if \code{compute.t1} is \code{TRUE}.
#'    \item \code{rescale.y.factor} If proxies (y-axis) should be rescaled, this can be done using this command. \code{y_new = y_old*rescale.y.factor}.
#'    \item \code{imp.fit} Boolean indicating whether initial values for \code{inla} be computed using \code{optim}.
#'   \item \code{log.theta.prior} Function argument which returns the logarithmic value of the joint prior of the rescaled hyperparameters \code{theta} used in the linear ramp model. If \code{NULL} (default) then default priors  will be assumed (see code in \code{rgeneric.r} file).
#'    \item \code{h} Step length of optimization procedure in the \code{inla}-program. Default value is \code{0.01}.
#'    \item \code{ncores} Integer giving the number of cores used in the \code{inla}-program. For \code{rgeneric} models which are used here, stability can sometimes be improved if the number of cores is set to \code{1}, which is why it is the default here.
#'    \item \code{silent} If equal to \code{TRUE} or \code{1L}, then \code{inla} will run in "silent" mode, If equal to \code{2L} then error messages will also be suppressed.
#'    \item \code{verbose} Boolean stating whether or not the \code{inla}-program should run in verbose mode. Default \code{FALSE}.
#' }
...
#'    \itemize{
#'      \item \code{label} Character used to describe the name of the transition in future \code{plot}. Default \code{NULL}.
#'      \item \code{nsims} Integer giving the number of samples to be generated for onset transition age. Default value is \code{10000}.
#'      \item \code{sync} Boolean indicating if the synchronized age-depth model should be used. Default value is \code{TRUE}, but this is set to \code{fALSE} if synchronized samples are not yet computed.
#'      \item \code{age.label} Character describing what label to place on x-axis (age) in a future \code{plot}. Default value is \code{""}.
#'      \item \code{age.ref} Numeric giving a reference value for the age which will be included in a future \code{plot}.
#'      }
#' @return Returns a list including default values for all variables in
