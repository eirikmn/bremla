...
#ggsave("illustrative-all-3000x4000.eps", device=cairo_ps,width=3000,height=4000,units="px",dpi=500,limitsize=FALSE,arrangeGrob(ggp,ggp2))
...
ggd = data.frame(y = res$data$dy,depth=res$data$depth,
  ...
  ggp = ggplot(data=ggd,aes(x=depth)) + theme_bw()+ #xlim(rev(range(results$data$depth)))+
    ...
    xlab("Depth (m)")+ylab("Layers per 1m")+ggtitle("(a) Fitted layer increment model")
...
ggsave("fitdiffsim-3000x2200.eps", device=cairo_ps,width=3000,height=2200,units="px",dpi=500,limitsize=FALSE,arrangeGrob(ggp,ggp2))
